...
public class LaunchControl extends Composite {
    ...
    public LaunchControl(Composite parent, int style) {
        ...
        label.setText("Базовая конфигурация запуска");
        ...
        label.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false, 1, 1));
        label.setText("Тестовое расширение");
    ...
    void initializeFrom() {
        UtilsUI.setValueSource(usedLaunchConfigurationControl, Arrays.asList(LaunchHelper.getLaunchConfigurations()));
        UtilsUI.setValueSource(
                testExtensionControl,
                LaunchHelper.getExtensions(),
                LabelProvider.createTextProvider(e ->
                        e == null ? "" : ((IExtensionProject) e).getDtProject().getName()
                )
        );
    }
}
